...
export function UnstakeForm({ poolId, pool, position }: UnstakeFormProps) {
  ...
  const handleUnstake = async () => {
    ...
    if (unstakeAmount > position.stakedAmount) {
      setError(`Maximum unstake amount is ${position.stakedAmount.toFixed(2)} SOL`);
      return;
    }

    setError('');
    await unstakeTokens(poolId, unstakeAmount);
  };
    ...
    <div className="space-y-6">
      {actionState.type === 'unstake' && actionState.success && (
        <div className="rounded-lg border border-emerald-700/50 bg-emerald-500/10 p-4 flex items-center gap-3">
          ...
          <div>
            <p className="font-semibold text-emerald-300">Unstaking initiated!</p>
            <p className="text-sm text-emerald-200">48-hour cooldown period started. Withdraw after cooldown completes.</p>
          </div>
        </div>
      )}
      ...
      <div className="rounded-lg border border-yellow-700/50 bg-yellow-500/10 p-4 flex items-start gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-yellow-200 mt-1">After unstaking, you'll need to wait 48 hours before you can withdraw your SOL.</p>
        </div>
      </div>

      <div>
        <label className="block text-sm font-medium text-slate-300 mb-2">Amount to Unstake (SOL)</label>
      ...
      {amount && parseFloat(amount) > 0 && (
        <div className="rounded-lg bg-white/5 p-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-white font-semibold">{parseFloat(amount).toFixed(2)} SOL</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Remaining Staked</span>
            <span className="text-white font-semibold">
              {Math.max(0, position.stakedAmount - parseFloat(amount)).toFixed(2)} SOL
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-slate-400">Accrued Rewards</span>
            <span className="text-emerald-400 font-semibold">{position.rewardsEarned.toFixed(4)} SOL</span>
          </div>
        </div>
      )}
